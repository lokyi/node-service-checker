...
_checks.post = function (data, callback) {
  ...
  let protocal = typeof data.payload.protocal === 'string' && ['https', 'http'].includes(data.payload.protocal.trim()) ? data.payload.protocal.trim() : false
...
// checks -put
// Required data: id
// Optional data: (at least one of) protocal, url, method, successCodes, timeoutSeconds
_checks.put = function (data, callback) {
  ...
  console.log(id, protocal, url, method, successCodes, timeoutSeconds )
